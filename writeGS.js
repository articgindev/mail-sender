import { google } from 'googleapis';

// Credenciales de Google (ten cuidado de no exponer estas credenciales públicamente)
const credentials = {
  type: "service_account",
  project_id: "web-artic",
  private_key_id: "a5ba27182dc75e5eb9a8bfa9481e733b2dd755cb",
  private_key: "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n",
  client_email: "id-artic@web-artic.iam.gserviceaccount.com",
  client_id: "100344301593881156249",
  auth_uri: "https://accounts.google.com/o/oauth2/auth",
  token_uri: "https://oauth2.googleapis.com/token",
  auth_provider_x509_cert_url: "https://www.googleapis.com/oauth2/v1/certs",
  client_x509_cert_url: "https://www.googleapis.com/robot/v1/metadata/x509/id-artic%40web-artic.iam.gserviceaccount.com"
};

const auth = new google.auth.GoogleAuth({
  credentials,  // Pasamos las credenciales directamente
  scopes: ['https://www.googleapis.com/auth/spreadsheets'],
});

// Función asíncrona para apendear datos en la hoja de Google Sheets
export async function writeToSheet(values) {
  const sheets = google.sheets({ version: 'v4', auth });
  const spreadsheetId = 'TU_SPREADSHEET_ID';  // Reemplaza con tu ID de hoja
  const range = 'Payments!A2';
  const valueInputOption = 'USER_ENTERED';

  const resource = { values };

  try {
    const response = await sheets.spreadsheets.values.append({
      spreadsheetId,
      range,
      valueInputOption,
      resource,
    });

    if (response.status === 200) {
      console.log('Datos añadidos a Google Sheets correctamente:', response.data);
    } else {
      throw new Error('No se pudieron añadir los datos a Google Sheets.');
    }
  } catch (error) {
    console.error('Error al escribir en Google Sheets:', error.message || error);
  }
}

// Ejemplo de prueba
(async () => {
  try {
    const values = [
      ['Pago ID', 'Estado', 'Monto', 'Fecha'] // Ejemplo de valores
    ];

    await writeToSheet(values);
    console.log('Prueba completada.');
  } catch (error) {
    console.error('Error en la prueba local:', error);
  }
})();
